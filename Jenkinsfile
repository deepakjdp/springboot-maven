pipeline {
    ...
    stages {
        ...
        stage('Build Frontend') {
            steps {
                echo '=========================================='
                echo 'Stage 5: Building Angular Frontend'
                echo '=========================================='
                dir('frontend') {
                    script {
                        sh '''
        ...
        stage('Frontend SonarQube Analysis') {
            steps {
                echo '=========================================='
                echo 'Stage 7: Running Frontend SonarQube Analysis'
                echo '=========================================='
                dir('frontend') {
                    script {
                        withSonarQubeEnv('SonarQube') {
                            sh '''
                                npm run sonar -- \
                                    -Dsonar.projectKey=springboot-angular-app-frontend \
                                    -Dsonar.host.url=${SONAR_HOST_URL} \
                                    -Dsonar.login=${SONAR_TOKEN} || echo "SonarQube scanner not configured"
                            '''
                        }
                    }
                }
